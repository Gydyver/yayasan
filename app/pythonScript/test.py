S_BOX = [
    
]

RCON = [
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
]

# AES-128 Encryption
def encrypt_AES_128(plaintext, key):
    state = [[0] * 4 for _ in range(4)]
    for i in range(4):
        for j in range(4):
            state[j][i] = plaintext[i * 4 + j]

    # Add Round Key (Initial Round)
    state = add_round_key(state, key)

    # 9 rounds
    for round_num in range(1, 10):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        key_schedule = generate_key_schedule(key, round_num)
        state = add_round_key(state, key_schedule)

    # Final Round
    state = sub_bytes(state)
    state = shift_rows(state)
    key_schedule = generate_key_schedule(key, 10)
    state = add_round_key(state, key_schedule)

    # Convert state matrix to ciphertext
    ciphertext = bytearray()
    for i in range(4):
        for j in range(4):
            ciphertext.append(state[j][i])

    return ciphertext

# Add Round Key Transformation
def add_round_key(state, key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= key[i * 4 + j]
    return state

# Sub Bytes Transformation
def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = S_BOX[state[i][j]]
    return state

# Shift Rows Transformation
def shift_rows(state):
    for i in range(1, 4):
        state[i] = state[i][i:] + state[i][:i]
    return state

# Mix Columns Transformation
def mix_columns(state):
    for i in range(4):
        s0 = state[0][i]
        s1 = state[1][i]
        s2 = state[2][i]
        s3 = state[3][i]

        state[0][i] = (mul2(s0) ^ mul3(s1) ^ s2 ^ s3)
        state[1][i] = (s0 ^ mul2(s1) ^ mul3(s2) ^ s3)
        state[2][i] = (s0 ^ s1 ^ mul2(s2) ^ mul3(s3))
        state[3][i] = (mul3(s0) ^ s1 ^ s2 ^ mul2(s3))

    return state

# Key Schedule Generation
def generate_key_schedule(key, round_num):
    key_schedule = [0] * 16
    for i in range(4):
        for j in range(4):
            key_schedule[i * 4 + j] = key[i * 4 + j]

    for i in range(4, 4 * (round_num + 1)):
        temp = key_schedule[(i - 1) * 4:i * 4]

        if i % 4 == 0:
            temp = temp[1:] + temp[:1]
            temp = [S_BOX[b] for b in temp]
            temp[0] ^= RCON[i // 4 - 1]
        elif i % 4 == 4:
            temp = [S_BOX[b] for b in temp]

        for j in range(4):
            key_schedule.append(key_schedule[(i - 4) * 4 + j] ^ temp[j])

    return key_schedule

# Multiplication by 2 in Galois Field (GF(2^8))
def mul2(num):
    return (num << 1) ^ (0x1b if (num & 0x80) else 0)

# Multiplication by 3 in Galois Field (GF(2^8))
def mul3(num):
    return mul2(num) ^ num

# Test
plaintext = bytearray(b'Hello, World!')
key = bytearray(b'abcdefghijklmnop')
ciphertext = encrypt_AES_128(plaintext, key)
print('Plaintext:', plaintext)
print('Ciphertext:', ciphertext)